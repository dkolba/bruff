...
- **AR-4 (MUST)** Accessibility checks run via `@axe-core/playwright` against the root game route and the dev-only `/tools` route in both light and dark colour schemes.
