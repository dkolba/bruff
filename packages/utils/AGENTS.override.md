...
- **U-4 (MUST)** PRNG helpers are pure value transformers. They accept explicit seed/state values, return the next state with the drawn value, and never read `Math.random()`, time, or module-level mutable state.
