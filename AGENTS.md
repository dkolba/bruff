...
- **O-11 (MUST)** Keep files ≤ 300 lines (comments do not count), and target ~200 lines where practical (Small Files Principle). Files larger than this almost always hide a missing module boundary (in rare cases, ask for permission to create larger files). Don't remove comments or TsDoc annotations to satisfy "File has too many lines" linting rule.
...
| `@bruff/quilt`         | Development-only map creation tool exposed only from Arcade dev routes               | `packages/quilt/AGENTS.md`        |
