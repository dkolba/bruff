...
- The Sigil tool downloads glyph maps whose top-level keys are stable glyph keys and whose entries contain the editable `name`, source glyph data, `mappedGlyph`, and exact `"LICENSE"` values.
...
- None. The migration target is the Sigil downloadable glyph map shape, including stable top-level keys and editable per-entry `name` values.
...
- Glyph names are stored in each glyph entry's `name` field.
