...
- [ ] T8 — Implement occupancy queries in `packages/game/lib/state/occupancy.ts`.
...
- [ ] T13 — Add enemy destination tests in `packages/game/lib/state/move-enemy-toward-player.test.ts` for horizontal priority, vertical priority, tie priority, and overlapping defensive input.
...
- [ ] T20 — Implement `migrateV1toV2` in `packages/game/lib/state/migrations.ts`.
