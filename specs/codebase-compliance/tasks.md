...
- [x] T48 — Add ambient declaration file `packages/game/lib/effects/observable/observable-polyfill.d.ts` augmenting `Document` with `when(eventName: string): Observable<Event>` and any other shape required by the polyfill. The pipeline operators (`filter`, `map`, `flatMap`, `take`) already live on the polyfill's exported `Observable` type, so re-importing it via `import type` is enough — the augmentation only needs to add `Document.when`. The file is picked up automatically by the existing `lib/**/*.ts` glob in `tsconfig.json` (`*.ts` matches `*.d.ts`). A bare `export {}` makes the file a module so the `import type` is legal; an inline `unicorn/require-module-specifiers` disable explains the bare export. T49 next uses this augmentation to drop the `as any` casts in `keydown.ts` and `touch.ts`.
...
- [ ] T51 — Add `@fast-check/vitest` as a `devDependency` of `@bruff/game`. Run `pnpm install`.
