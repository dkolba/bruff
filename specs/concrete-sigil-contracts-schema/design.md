...
`namesByUnicode` is still used to label extracted rows and to populate the exported glyph entry `name` field. Required glyph row labels come from `requiredGlyphSelections` so a contract glyph can choose any typed source character, and required names remain the exported top-level keys.
...
- `packages/sigil/module/tool-sigil.test.ts`, split state tests under `packages/sigil/module/state/`, `tool-sigil-bindings.test.ts`, `tool-sigil-render.test.ts`, `tool-sigil-download.test.ts`, and `tool-sigil-error.test.ts` cover component, state, binding, render, download, and validation behaviour.
...
completeMissingDrafts(font, selected required characters, drafts)
        |
apply required glyph names as top-level keys
        |
store edited glyph-name input values in glyph name fields
        |
...
- Add download tests proving edited `glyph-name-...` inputs appear as glyph `name` fields in exported JSON while top-level required keys stay stable.
